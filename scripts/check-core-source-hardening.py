...
HOST_CONTEXT_MARKERS: list[tuple[str, str]] = [
    ...
    ("hostname_helper", "HOSTNAME = Path(\"/usr/bin/hostname\")"),
    ("tailscale_helper", "TAILSCALE = Path(\"/usr/bin/tailscale\")"),
    ("parse_context", "def parse_context(path: Path)"),
    ("read_text_context", "path.read_text(encoding=\"utf-8\", errors=\"replace\")"),
    ("hostname_command", "subprocess.run([str(HOSTNAME)]"),
    ("tailscale_command", "subprocess.run([str(TAILSCALE), \"ip\", \"-4\"]"),
...
TIME_SYNC_MARKERS: list[tuple[str, str]] = [
    ...
    ("timedatectl_helper", "TIMEDATECTL = Path(\"/usr/bin/timedatectl\")"),
    ("chronyc_helper", "CHRONYC = Path(\"/usr/bin/chronyc\")"),
...
PRIVILEGE_POLICY_MARKERS: list[tuple[str, str]] = [
    ...
    ("sudo_helper", "SUDO = Path(\"/usr/bin/sudo\")"),
    ("sudo_is_usable", "def sudo_is_usable() -> bool:"),
    ("sudo_list", "subprocess.run([str(SUDO), \"-n\", \"-l\", \"-U\", TARGET_USER]"),
