...
PYTHON_SYNTAX_MARKERS: list[tuple[str, str]] = [
    ...
    ("python_env", "PYTHON_SYNTAX_SUMMARY=\"$summary\" \"$PYTHON_BIN\" - <<'PY'"),
...
SHELL_SYNTAX_MARKERS: list[tuple[str, str]] = [
    ...
    ("bash_n", "\"$BASH_BIN\" -n \"$path\""),
    ("is_shell_script", "is_shell_script()"),
    ("shebang", "'#!'*'/bash'"),
    ("find_scripts", "\"$FIND_BIN\" scripts -maxdepth 1 -type f"),
    ("maxdepth", "-maxdepth 1"),
    ("sort", "| \"$SORT_BIN\""),
...
SYSTEMD_UNIT_MARKERS: list[tuple[str, str]] = [
    ...
    ("unit_policy_failures", "unit_policy_failures=0"),
    ("parent_policy_failures", "parent_policy_failures=0"),
    ("attr_policy_failures", "attr_policy_failures=0"),
    ("lsattr_bin", "LSATTR_BIN=\"/usr/bin/lsattr\""),
    ("getfacl_bin", "GETFACL_BIN=\"/usr/bin/getfacl\""),
    ("getfattr_bin", "GETFATTR_BIN=\"/usr/bin/getfattr\""),
    ("checks_counter", "checks=0"),
    ("project_unit_owner", "project_unit_owner=\"$($STAT_BIN -c '%U' \"${ROOT}/systemd\")\""),
    ("project_unit_group", "project_unit_group=\"$($STAT_BIN -c '%G' \"${ROOT}/systemd\")\""),
    ("parent_directory_policy", "check_parent_directory_policy()"),
    ("project_owner_scope", "project:systemd follows the project tree owner/group"),
    ("installed_parent_scope", "root:root requirement is intentionally limited to the direct installed"),
    ("vendor_runtime_user_units_excluded", "Vendor units below /usr/lib/systemd"),
    ("attr_policy", "check_attr_policy()"),
    ("attr_policy_summary", "systemd_attr_policy=%s status=ok flags=%s"),
    ("attr_policy_unexpected", "systemd_attr_policy=%s status=unexpected flags=%s unexpected=%s"),
    ("lsattr_missing", "systemd_attr_policy=%s status=lsattr_missing"),
    ("lsattr_call_dir", "$LSATTR_BIN -d \"$path\""),
    ("lsattr_call_file", "$LSATTR_BIN \"$path\""),
    ("allowed_extents_flag", "unexpected=\"${unexpected//e/}\""),
    ("acl_availability", "acl_tool_available=%d"),
    ("xattr_availability", "xattr_tool_available=%d"),
    ("parent_symlink_policy", "systemd_parent_policy=%s status=symlink"),
    ("parent_regular_policy", "systemd_parent_policy=%s status=not_directory"),
    ("parent_world_writable_policy", "systemd_parent_policy=%s status=world_writable"),
    ("parent_owner_policy", "systemd_parent_policy=%s status=owner_unexpected"),
    ("unit_file_policy", "check_unit_file_policy()"),
    ("direct_unit_scope", "direct /etc/systemd/system/<unit> files listed above"),
    ("wants_links_excluded", "enablement links below *.wants/ are not part of this guard's path set"),
    ("symlink_policy", "status=symlink"),
    ("regular_policy", "status=not_regular"),
    ("world_writable_policy", "status=world_writable"),
    ("stat_mode", "$STAT_BIN -c '%a'"),
    ("stat_owner", "$STAT_BIN -c '%U'"),
    ("stat_group", "$STAT_BIN -c '%G'"),
    ("installed_owner_policy", "systemd_unit_policy=%s status=owner_unexpected"),
    ("project_owner_policy", "systemd_unit_policy=%s status=project_owner_unexpected"),
    ("project_parent_policy_call", "check_parent_directory_policy \"${ROOT}/systemd\" \"project:systemd\" \"project\""),
    ("etc_systemd_policy_call", "check_parent_directory_policy \"/etc/systemd\" \"installed:/etc/systemd\" \"installed\""),
    ("etc_systemd_system_policy_call", "check_parent_directory_policy \"/etc/systemd/system\" \"installed:/etc/systemd/system\" \"installed\""),
    ("src", "src=\"${ROOT}/systemd/${unit}\""),
    ("dst", "dst=\"/etc/systemd/system/${unit}\""),
    ("cmp", "\"$CMP_BIN\" -s \"$src\" \"$dst\""),
    ("project_policy", "check_unit_file_policy \"$src\" \"project:${unit}\" \"project\""),
    ("installed_policy", "check_unit_file_policy \"$dst\" \"installed:${unit}\" \"installed\""),
    ("systemctl_active", "$SYSTEMCTL_BIN is-active \"$timer\""),
    ("systemctl_failed", "$SYSTEMCTL_BIN is-failed \"$service\""),
    ("inactive_timers", "inactive_timers=0"),
    ("failed_services", "failed_services=0"),
    ("summary", "systemd_unit_guard_status=ok checks=%d units=%d timers=%d services=%d sync_failures=0 missing_units=0 unit_policy_failures=0 parent_policy_failures=0 attr_policy_failures=0 lsattr_available=%d acl_tool_available=%d xattr_tool_available=%d inactive_timers=0 failed_services=0"),
]
...
SCRIPT_PERMISSION_POLICY_MARKERS: list[tuple[str, str]] = [
    ...
    ("systemd_path", "SYSTEMD_DIR = ROOT / \"systemd\""),
    ("docs_path", "DOCS_DIR = ROOT / \"docs\""),
    ("expected_dirs", "EXPECTED_PROJECT_DIRS = [ROOT, SCRIPTS, SYSTEMD_DIR, DOCS_DIR]"),
    ("expected_systemd", "EXPECTED_SYSTEMD_FILES = {"),
    ("expected_docs", "EXPECTED_DOC_FILES = {"),
    ("non_executable_sources", "NON_EXECUTABLE_SCRIPT_SOURCES = {"),
    ("top_level_sources", "TOP_LEVEL_SOURCE_NAMES = {"),
    ("direct_files", "def direct_files(path: Path) -> list[Path]:"),
    ("common_policy", "def check_common_file_policy("),
    ("directory_policy", "def check_directory_policy("),
    ("lstat", "path.lstat()"),
    ("mode", "stat.S_IMODE"),
    ("world_writable", "world_writable"),
    ("symlinks", "symlinks"),
    ("owner_mismatches", "owner_mismatches"),
    ("executable_failures", "executable_policy_failures"),
    ("group_writable", "group_writable_sources"),
    ("summary", "script_permission_policy_status=%s checks=%d findings=%d script_files=%d executable_scripts=%d"),
]
